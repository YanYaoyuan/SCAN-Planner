...
class LegacyZsiBotOptInTest(unittest.TestCase):
    ...
    def test_legacy_binaries_have_their_own_opt_in_guard(self):
        ...
        runtime_guard = read(
            "tools/orin_runtime/assert_no_legacy_zsibot_owner.sh"
        )
        self.assertIn(
            "zsibot_cmd_bridge|zsibot_cmd_udp_client|zsibot_sdk_proxy",
            runtime_guard,
        )
        self.assertNotIn(
            '"${install_prefix}/lib/zsibot_cmd_bridge/zsibot_cmd_udp_client"',
            runtime_guard,
        )
    ...
    def test_stale_installed_sdk_owner_fails_closed(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            compat_udp_client = (
                artifact_prefix
                / "lib/zsibot_cmd_bridge/zsibot_cmd_udp_client"
            )
            compat_udp_client.parent.mkdir(parents=True)
            compat_udp_client.touch()
            ...
            legacy_client_running = subprocess.run(
                ["bash", str(guard), str(artifact_prefix)],
                check=False,
                capture_output=True,
                text=True,
                env=env,
            )
            self.assertEqual(legacy_client_running.returncode, 4)
            self.assertIn(
                "deprecated ZsiBot control process", legacy_client_running.stderr
            )
            self.assertIn("4242", legacy_client_running.stderr)
