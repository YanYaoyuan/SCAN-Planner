...
- `navi_mode=3` 现在把输入路径作为不可被局部重规划覆盖的全局路线：
  - 去除重复点和与当前位姿重合的零长度首段；
  - 按路径弧长单调推进局部目标，路径交叉或闭环终点不会造成进度跳跃；
  - 每次截取前方参考路径片段初始化局部 B-spline，并用软约束抑制急弯切角；
  - 碰撞代价仍可让 B-spline 为避障偏离路线，模式 1/2 的优化行为不变；
  - 重规划不再退化为当前位置直达终点；
  - 开放路径和闭环路径都必须消耗完剩余路径弧长并接近末点后才算完成；
  - 路径早于首个合法机体里程计到达时会缓存，而不是丢弃。
